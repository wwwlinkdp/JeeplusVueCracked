	...
	<view>
		...
		<form @submit="formSubmit" class="cu-list menu">
			...
			<view class="cu-form-group">
				...
				<input placeholder="请输入标题" maxlength="200" v-model="inputForm.mailDTO.title" name="title"></input>
			</view>
			<view class="cu-form-group">
				<view class="title">
					<text class="red-color">* </text> 内容
				</view>
				<textarea maxlength="2000"  v-model="inputForm.mailDTO.content" name="content" placeholder="请填写内容"></textarea>
			</view>
			<view class="bottom-wrap  flex">
				<view class="flex-sub">
					<button class="cu-btn lg block buttonBox  bg-gray" @click="saveDraft"> 存为草稿</button>
				</view>
				<view class="flex-sub">
					<button class="cu-btn lg   buttonBox block bg-blue " @click="sendEmail">发送邮件</button>
				</view>
...
<script>
	...
	export default {
		onShow() {
		  ...
		  this.userService.treeData().then(({data})=>{
			  this.data = data
		  }).catch((e)=>{
			  throw e
		  })
		...
		async onLoad(mail) {
			if(mail&&mail.id){
				this.titile = "编辑邮件";
				let {data} = await this.mailComposeService.queryById(mail.id);
		...
		methods: {
			...
			formSubmit: function(e) {
				...
				var rule = [
					...
					{name:"mailDTO.title", checkType : "notnull", checkRule:"",  errorMsg:"标题不能为空"},
					{name:"mailDTO.content", checkType : "notnull", checkRule:"",  errorMsg:"内容不能为空"}
				];
				//进行表单检查
