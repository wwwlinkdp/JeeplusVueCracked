     ...
     <div class="bg-white top">
        <vxe-toolbar :refresh="{query: refreshList}" export print custom>
          <template #buttons>
            ...
            <el-button v-if="hasPermission('test:activiti:testActivitiAudit:edit')" type="warning" size="small" icon="el-icon-edit-outline" @click="edit()" :disabled="$refs.testActivitiAudit && $refs.testActivitiAudit.getCheckboxRecords().length !== 1" plain>修改</el-button>
            <el-button v-if="hasPermission('test:activiti:testActivitiAudit:del')" type="danger"   size="small" icon="el-icon-delete" @click="del()" :disabled="$refs.testActivitiAudit && $refs.testActivitiAudit.getCheckboxRecords().length === 0" plain>删除</el-button>
          </template>
        </vxe-toolbar>
...
<script>
  ...
  export default {
    ...
    testActivitiAuditService: null,
    created () {
      this.testActivitiAuditService = new TestActivitiAuditService()
    },
    activated () {
      this.refreshList()
    },
    methods: {
      ...
      refreshList () {
        this.loading = true
        this.testActivitiAuditService.list({
      ...
      del (id) {
        ...
        this.$confirm(`确定删除所选项吗?`, '提示', {
          ...
          this.loading = true
          this.testActivitiAuditService.delete(ids).then(({data}) => {
      ...
      detail ({row}) {
        return new Promise(resolve => {
          this.testActivitiAuditService.queryById(row.id).then(({data}) => {
