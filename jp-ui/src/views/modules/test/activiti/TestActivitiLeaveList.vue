     ...
     <div class="bg-white top">
        <vxe-toolbar :refresh="{query: refreshList}" export print custom>
          <template #buttons>
            ...
            <el-button v-if="hasPermission('test:activiti:testActivitiLeave:edit')" type="warning" size="small" icon="el-icon-edit-outline" @click="edit()" :disabled="$refs.testActivitiLeave && $refs.testActivitiLeave.getCheckboxRecords().length !== 1" plain>修改</el-button>
            <el-button v-if="hasPermission('test:activiti:testActivitiLeave:del')" type="danger"   size="small" icon="el-icon-delete" @click="del()" :disabled="$refs.testActivitiLeave && $refs.testActivitiLeave.getCheckboxRecords().length === 0" plain>删除</el-button>
          </template>
        </vxe-toolbar>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      refreshList () {
        this.loading = true
        this.testActivitiLeaveService.list({
      ...
      del (id) {
        ...
        this.$confirm(`确定删除所选项吗?`, '提示', {
          ...
          this.loading = true
          this.testActivitiLeaveService.delete(ids).then(({data}) => {
      ...
      detail ({row}) {
        return new Promise(resolve => {
          this.testActivitiLeaveService.queryById(row.id).then(({data}) => {
